...
const Sidebar = ({recipes,handlePreparingRecipe,totalTime,totalCalories}) => {
    return (
        <div>
                        ...
                        <tbody  className='bg-base-200'>
                            {
                                recipes.map((recipe,idx)=> <tr key={idx}>
                                    ...
                                    <td><button onClick={()=>handlePreparingRecipe(recipe.preparing_time,recipe.calories)} className="btn border-none rounded-full bg-[#0BE58A] text-black px-4">Preparing..</button></td>
                                </tr>)
                            }
                        </tbody>
                    </table>
